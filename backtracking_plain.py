...
def recursive_backtracking_search(ta_capacity, course_capacity_req, solution):
    ...
    for ta in ta_capacity:
        ...
        if check_constraint(ta, unassigned_course, num_skill_req, solution):
            available_ta_capacity = get_available_ta_capacity(ta[0], solution)
            if available_ta_capacity == 0.0:
                continue
            # create copy
            ...
            if unassigned_course[1] > available_ta_capacity:
                filled_capacity = available_ta_capacity
                course_cap_req_copy[course_cap_req_copy.index(unassigned_course)][1] -= filled_capacity

            elif unassigned_course[1] < available_ta_capacity:
    ...
    if len(solution_list) == 0:
        ...
        for ta in ta_capacity:
            if get_available_ta_capacity(ta[0], solution) > 0:
        ...
        return False
"""
During the assignment of TA, it has to be checked if the TA which is being considered for assignment has already been
assigned previously so that only remaining capacity is considered during this current assignment. This method provides
the available TA for capacity for a TA based on assignment done so far.
"""

def get_available_ta_capacity(ta, solution):
    consumed_ta_capacity = 0.0
    for course, assignment in solution.items():
        for capacity in assignment:
            if capacity[0] == ta:
                consumed_ta_capacity += capacity[1]

    return 1 - consumed_ta_capacity
